...
class Crypto extends Component {
    ...
    async checkLastTransaction() {
        ...
        try {
            const historyUSDC = await this.context.value.connection.getConfirmedSignaturesForAddress2(this.context.value.cryptoAccounts.usdc, { limit: 1 });
    ...
    async syncCrypto() {
        ...
        res.forEach(element => {
            ...
            let mult = element.transaction.message.accountKeys[0] === this.context.value.wallet.publicKey.toBase58() ? -1 : 1
            temp.push({
        ...
        return (
            <View>
                <View style={{ flex: 1, flexDirection: "column", justifyContent: "center", alignItems: "center" }}>
                    ...
                    <Text style={{ color: "blue", textDecorationLine: "underline", fontSize: 20, textAlign: "justify"}} onPress={() => Linking.openURL(`https://solscan.io/account/${this.context.value.wallet.publicKey.toBase58()}`)}>
                        {
                    ...
                    <ScrollView>
                        {
                            this.state.transactions && this.state.transactions.map((item, index) =>
                                <View key={index} style={{ flexDirection: "row", alignContent: "space-between", alignItems: "center", paddingTop: 10 }}>
                                    <Text style={{ color: "white", fontSize: 20, textAlign: "center", width: Dimensions.get("window").width * .333 }}>
                                        {item.pay ? "Solana Pay" : "Transfer"}
                                    </Text>
                                    <Text style={{ color: item.amount >= 0 ? "green" : "red", fontSize: 20, textAlign: "center", width: Dimensions.get("window").width * .333 }}>
                                        {item.icon} {" "}
